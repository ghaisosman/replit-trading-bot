...
class OrphanGhostDetectionTest:
    ...
    def _test_orphan_detection(self):
        ...
        for i, strategy in enumerate(self.test_strategies[:3]):  # Test 3 strategies
    ...
    def _test_orphan_clearing(self):
        ...
        clearing_strategies = ['macd_divergence', 'engulfing_pattern', 'rsi_oversold']  # Test all strategies

        # Test clearing for each detected orphan
        for strategy in clearing_strategies:
            orphan_id = f"{strategy}_{self.test_symbols[0]}" # Assuming same symbol for all
            if orphan_id not in self.trade_monitor.orphan_trades:
                print(f"\n   ⚠️ No orphan trade found for {strategy}, skipping clearing test.")
                clearing_results[strategy] = {
                    'monitor_cleared': None,
                    'position_cleared': None,
                    'database_updated': None,
