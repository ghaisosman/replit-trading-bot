...
def create_bullish_engulfing_scenario(rsi_value=45):
    ...
    # Generate price trend that leads to desired RSI
    price_changes = []
    for i in range(48):
        # Create price movements that will result in target RSI
        if i < 30:
            change = np.random.uniform(-0.5, 0.5)  # Neutral movements
        else:
            # Create downward pressure for low RSI
            change = np.random.uniform(-1.5, 0.5) if rsi_value < 50 else np.random.uniform(-0.5, 1.5)
        price_changes.append(change)

    # Build normal candles with price trend
    current_price = base_price
    for i in range(48):
        current_price += price_changes[i] * 50  # Scale the change
        ...
        close_price = current_price + np.random.uniform(-25, 25)
    ...
    # Add clear bearish candle (previous)
    prev_open = current_price + 50
    prev_close = current_price - 50  # Clear bearish candle
    data.append({
        'timestamp': dates[48],
        'open': prev_open,
        'high': prev_open + 20,
    ...
    # Add clear bullish engulfing candle (current)
    curr_open = prev_close - 30   # Well below previous close
    curr_close = prev_open + 30   # Well above previous open - CLEAR ENGULFING
    data.append({
        'timestamp': dates[49],
        'open': curr_open,
        'high': curr_close + 20,
...
def create_bearish_engulfing_scenario(rsi_value=55):
    ...
    for i in range(48):
        ...
        if i < 30:
            ...
            # Create upward pressure for high RSI
            change = np.random.uniform(-0.5, 1.5) if rsi_value > 50 else np.random.uniform(-1.5, 0.5)
        price_changes.append(change)
    ...
    # Add clear bullish candle (previous)
    prev_open = current_price - 50
    prev_close = current_price + 50  # Clear bullish candle
    data.append({
        'timestamp': dates[48],
        'open': prev_open,
        'high': prev_close + 20,
    ...
    # Add clear bearish engulfing candle (current)
    curr_open = prev_close + 30   # Well above previous close
    curr_close = prev_open - 30   # Well below previous open - CLEAR ENGULFING
    data.append({
        'timestamp': dates[49],
        'open': curr_open,
        'high': curr_open + 20,
